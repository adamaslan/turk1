...
// Separate TTS endpoint for source language
export async function PUT(req: Request) {
  try {
    ...
    return new Response(audio.audioContent, {
      headers: {
        'Content-Type': 'audio/mpeg',
        'Content-Lengtimport { NextResponse } from 'next / server';
import { v3 } from '@google-cloud/translate';
        import textToSpeech from '@google-cloud/text-to-speech';
    ...
    } h': Buffer.byteLength(audio.audioContent as Buffer),
      },
    });
  } catch (err: any) {
    console.error('TTS API Error:', err);
